...
(function (window, document, $) {
    ...
    $(".mainmn").on('click', function (event) {
        event.preventDefault();
        $('ul[data-menu="smenu"]').removeClass('active');
        var node = "#" + $(this).attr('targe-metnu');
        $(node).addClass("active");
    });
    $(".dnhsmap").on('click', function (event) {
        $("#panelL_menuPanel_subModulesBar li").css('display', 'none');

        var node = $("li[parent-node=" + $(this).attr('targe-metnu') + "]");
        if (node.length >= 2) {
            $(node).removeAttr("style");


        }
        var activeTab = $(node).find(".toolSubModPushed").parent().parent();

        $("div.scsubmn").css('display', 'none');
        LoadMenu(activeTab);
...
function LoadMenu(liobj) {
    var Child = $("li[parent-node=" + $(liobj).attr('parent-node') + "] .dnhtabmenu");

    $(Child).removeClass('toolSubModPushed').addClass("toolSubModNormal");
    //$(Child).removeClass('toolSubModNormal');

    $(liobj).find(".dnhtabmenu").addClass("toolSubModPushed");
    var parentid = $(liobj).attr('nodeid');
    ...
    if (bz.length == 0) {
        ...
        $("#seccond_menu").append($("<div></div>").load(GetPath("/SiteMap/SecondMenu?NodeID=" + parentid), function () {
            $(".DNHRemove").removeClass("DNHDefault");
            $('.mysitemap').unbind();
            $('.mysitemap').click(function (e) {
                e.preventDefault();
                BindSiteMap(this, e);
            });
            $("#sml" + parentid).removeAttr("style");
            //BindDefault(false)
        }));
    } else {


        $("#sml" + parentid).css('display', 'block');
        $("#sml" + parentid).removeAttr("style");
    }
}
