...
namespace Biz.Core.Services
{
    ...
    {
        #region Constants
        private const string SETTINGS_ALL_KEY = "ifinds.Models.AccessRights.all{0}";
        private const string SETTINGS_ID_KEY = "ifinds.Models.AccessRights.{0}{1}";
		private const string SETTINGS_User_KEY = "ifinds.Models.AccessRights.USer.{0}{1}";
		private const string SETTINGS_Search_KEY = "ifinds.Models.AccessRights.Search.{0}";
        private const string Resource = "AccessRights";
        #endregion
        ...
        public static AccessRights GetById(string id,int CompanyID)
        {
            string key = String.Format(SETTINGS_ID_KEY, id,CompanyID);
            object obj2 = HttpCache.Get(key);
            if (obj2 != null) { return (AccessRights)obj2; }

            AccessRights b = new AccessRights();
            using (var client = WebApiHelper.myclient(HouseEndpoint, SystemConst.APIJosonReturnValue))
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    b = response.Content.ReadAsAsync<AccessRights>().GetAwaiter().GetResult();
                }
            }
            HttpCache.Max(key, b);
            return b;
        ...
        public static AccessRights Add(AccessRights objItem)
        {
            AccessRights b = new AccessRights();
            using (var client = WebApiHelper.myclient(HouseEndpoint, SystemConst.APIJosonReturnValue))
            {
        ...
        public static void Delete(AccessRights objItem)
        {
            if (objItem != null)
            {
                using (var client = WebApiHelper.myclient(HouseEndpoint, SystemConst.APIJosonReturnValue))
        ...
        public static AccessRights Update(AccessRights objItem)
        {

            AccessRights item = new AccessRights();
            using (var client = WebApiHelper.myclient(HouseEndpoint, SystemConst.APIJosonReturnValue))
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    item = response.Content.ReadAsAsync<AccessRights>().GetAwaiter().GetResult();
                    RemoveCache(item);
                }
            }
            return item;
        ...
        public static void RemoveCache(AccessRights objItem)
        {
        ...
        public static string HouseEndpoint
        {
		 ...
		 #region new method
        public static AccessRightsCollection GetAllByUser(string CreatedUser,int CompanyID)
        {
            AccessRightsCollection items = new AccessRightsCollection();
